import { useState } from "react";

function Header({ changeDarkMode, darkMode, lingua, isVisible }) {

    // console.log(isVisible);

    const [ hamburguer, setHamburguer ] = useState(false)

    const handleHamburguer = () => {
        hamburguer ? setHamburguer(false) : setHamburguer(true)
    }

    return ( 
        <div className="z-30 lg:fixed top-10 left-32 flex flex-col lg:justify-between mx-10 lg:mx-auto py-20 lg:w-1/3 lg:h-[100vh]">
            <div>
                <h1 className={`${darkMode ? 'text-slate-300' : 'text-sky-900'} text-4xl font-bold`}>Lorenzo Adinolfi Acquesta</h1>
                <h2 className={`${darkMode ? 'text-slate-200' : 'text-sky-900'} text-xl my-4`}>Engenheiro de software</h2>
                <p className={`w-1/2 mt-4 ${darkMode ? 'text-slate-400' : 'text-indigo-950'} text-lg leading-normal`}>Desenvolvo projetos online completos, aplicações e designs</p>
            </div>
            <div>
                <button onClick={() => changeDarkMode()} className="text-slate-300 mt-5">
                    {
                        darkMode ? (
                            <svg width="30px" height="30px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M18 12C18 15.3137 15.3137 18 12 18C8.68629 18 6 15.3137 6 12C6 8.68629 8.68629 6 12 6C15.3137 6 18 8.68629 18 12Z" fill="rgb(148 163 184)"/>
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M12 1.25C12.4142 1.25 12.75 1.58579 12.75 2V3C12.75 3.41421 12.4142 3.75 12 3.75C11.5858 3.75 11.25 3.41421 11.25 3V2C11.25 1.58579 11.5858 1.25 12 1.25ZM4.39861 4.39861C4.6915 4.10572 5.16638 4.10572 5.45927 4.39861L5.85211 4.79145C6.145 5.08434 6.145 5.55921 5.85211 5.85211C5.55921 6.145 5.08434 6.145 4.79145 5.85211L4.39861 5.45927C4.10572 5.16638 4.10572 4.6915 4.39861 4.39861ZM19.6011 4.39887C19.894 4.69176 19.894 5.16664 19.6011 5.45953L19.2083 5.85237C18.9154 6.14526 18.4405 6.14526 18.1476 5.85237C17.8547 5.55947 17.8547 5.0846 18.1476 4.79171L18.5405 4.39887C18.8334 4.10598 19.3082 4.10598 19.6011 4.39887ZM1.25 12C1.25 11.5858 1.58579 11.25 2 11.25H3C3.41421 11.25 3.75 11.5858 3.75 12C3.75 12.4142 3.41421 12.75 3 12.75H2C1.58579 12.75 1.25 12.4142 1.25 12ZM20.25 12C20.25 11.5858 20.5858 11.25 21 11.25H22C22.4142 11.25 22.75 11.5858 22.75 12C22.75 12.4142 22.4142 12.75 22 12.75H21C20.5858 12.75 20.25 12.4142 20.25 12ZM18.1476 18.1476C18.4405 17.8547 18.9154 17.8547 19.2083 18.1476L19.6011 18.5405C19.894 18.8334 19.894 19.3082 19.6011 19.6011C19.3082 19.894 18.8334 19.894 18.5405 19.6011L18.1476 19.2083C17.8547 18.9154 17.8547 18.4405 18.1476 18.1476ZM5.85211 18.1479C6.145 18.4408 6.145 18.9157 5.85211 19.2086L5.45927 19.6014C5.16638 19.8943 4.6915 19.8943 4.39861 19.6014C4.10572 19.3085 4.10572 18.8336 4.39861 18.5407L4.79145 18.1479C5.08434 17.855 5.55921 17.855 5.85211 18.1479ZM12 20.25C12.4142 20.25 12.75 20.5858 12.75 21V22C12.75 22.4142 12.4142 22.75 12 22.75C11.5858 22.75 11.25 22.4142 11.25 22V21C11.25 20.5858 11.5858 20.25 12 20.25Z" fill="rgb(148 163 184)"/>
                            </svg>
                        ) : (
                            <svg width="30px" height="30px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M12 22C17.5228 22 22 17.5228 22 12C22 11.5373 21.3065 11.4608 21.0672 11.8568C19.9289 13.7406 17.8615 15 15.5 15C11.9101 15 9 12.0899 9 8.5C9 6.13845 10.2594 4.07105 12.1432 2.93276C12.5392 2.69347 12.4627 2 12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22Z" fill="#1C274C"/>
                            </svg>
                            
                        )
                    }
                </button>
            </div>
            <div className="lg:hidden fixed top-10 right-0 px-10 flex flex-col justify-center items-center">
                <div onClick={() => handleHamburguer()} className={`flex flex-col gap-2 w-12 h-12 items-center justify-center cursor-pointer ${darkMode ? 'bg-slate-700' : 'bg-sky-900'} bg-slate-700 rounded-full`}>
                    <div className={`w-6 h-[1px] bg-slate-300`}></div>
                    <div className={`w-6 h-[1px] bg-slate-300`}></div>
                    <div className={`w-6 h-[1px] bg-slate-300`}></div>
                </div>


                <div className={`${hamburguer ? 'opacity-100' : 'opacity-0'} absolute top-14 ${darkMode ? 'bg-slate-700' : 'bg-sky-900'} py-3 px-3 rounded-md transition-all`}>
                    <div className={`relative -top-4 w-4 h-4 ${darkMode ? 'bg-slate-700' : 'bg-sky-900'} mx-auto rotate-45`} ></div>
                    <a href="#sobre"  className={`z-30 flex items-center gap-2 uppercase font-bold text-xs text-slate-400 cursor-pointer transition-all ${isVisible.sobre ? 'text-slate-100 w-full' : ''}`} >
                        <p className={`mx-auto`}>Sobre</p>
                    </a>
                    <a href="#experiencia" className={`z-30  mt-4 flex items-center gap-2 uppercase font-bold text-xs text-slate-400 cursor-pointer transition-all ${isVisible.cardInfos ? 'text-slate-100 w-full' : ''}`}>
                        <p className={`mx-auto`}>Experiência</p>
                    </a>
                    
                    <a href="#projetos" className={`z-30  mt-4 flex items-center gap-2 uppercase font-bold text-xs text-slate-400 cursor-pointer transition-all ${isVisible.cardProjetos ? 'text-slate-100 w-full' : ''}`}>
                        <p className={`mx-auto`}>Projetos</p>
                    </a>
                </div>
            </div>
            <div className="hidden lg:block">
                    <a href="#sobre"  className={`z-30 w-1/3 mt-4 flex items-center gap-2 uppercase font-bold text-xs text-slate-400 cursor-pointer hover:text-slate-300 hover:w-full transition-all ${isVisible.sobre ? 'text-slate-100 w-full' : ''}`} >
                        <div className={`${darkMode ? 'bg-slate-400' : 'bg-sky-800'} bg-slate-400 w-1/6 h-[1px]`}></div>
                        <p className={`${darkMode ? 'text-slate-400' : 'text-indigo-950'}`}>Sobre</p>
                    </a>
                    <a href="#experiencia" className={`z-30 w-1/3 mt-4 flex items-center gap-2 uppercase font-bold text-xs text-slate-400 cursor-pointer hover:text-slate-300 hover:w-full transition-all ${isVisible.cardInfos ? 'text-slate-100 w-full' : ''}`}>
                        <div className={`${darkMode ? 'bg-slate-400' : 'bg-sky-800'} bg-slate-400 w-1/6 h-[1px]`} ></div>
                        <p className={`${darkMode ? 'text-slate-400' : 'text-indigo-950'}`}>Experiência</p>
                    </a>
                    
                    <a href="#projetos" className={`z-30 w-1/3 mt-4 flex items-center gap-2 uppercase font-bold text-xs text-slate-400 cursor-pointer hover:text-slate-300 hover:w-full transition-all ${isVisible.cardProjetos ? 'text-slate-100 w-full' : ''}`}>
                        <div className={`${darkMode ? 'bg-slate-400' : 'bg-sky-800'} bg-slate-400 w-1/6 h-[1px]`} ></div>
                        <p className={`${darkMode ? 'text-slate-400' : 'text-indigo-950'}`}>Projetos</p>
                    </a>
{/*                     
                    <a href="#contato" className="w-1/3 mt-4 flex items-center gap-2 uppercase font-bold text-xs text-slate-400 cursor-pointer hover:text-slate-300 hover:w-full transition-all">
                        <div className={`${darkMode ? 'bg-slate-400' : 'bg-sky-800'} bg-slate-400 w-1/6 h-[1px]`} ></div>
                        <p className={`${darkMode ? 'text-slate-400' : 'text-indigo-950'}`}>Contato</p>
                    </a> */}
            </div>
            <div>
                <ul className="flex gap-5 mt-5">
                    <li className="cursor-pointer">
                        <a target="_blank" href="https://www.linkedin.com/in/lorenzo-acquesta/">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill={`${darkMode ? '#cbd5e1' : '#1e1b4b'}`} className="h-6 w-6" aria-hidden="true"><path d="M20.5 2h-17A1.5 1.5 0 002 3.5v17A1.5 1.5 0 003.5 22h17a1.5 1.5 0 001.5-1.5v-17A1.5 1.5 0 0020.5 2zM8 19H5v-9h3zM6.5 8.25A1.75 1.75 0 118.3 6.5a1.78 1.78 0 01-1.8 1.75zM19 19h-3v-4.74c0-1.42-.6-1.93-1.38-1.93A1.74 1.74 0 0013 14.19a.66.66 0 000 .14V19h-3v-9h2.9v1.3a3.11 3.11 0 012.7-1.4c1.55 0 3.36.86 3.36 3.66z"></path></svg>
                        </a>
                    </li>
                    <li className="cursor-pointer">
                        <a target="_blank" href="https://www.github.com/acquesta">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill={`${darkMode ? '#cbd5e1' : '#1e1b4b'}`} className="h-6 w-6" aria-hidden="true"><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z"></path></svg>             
                        </a>
                    </li>
                    <li className="cursor-pointer">
                        <a>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960" width="24px" fill={`${darkMode ? '#cbd5e1' : '#1e1b4b'}`}><path d="M160-160q-33 0-56.5-23.5T80-240v-480q0-33 23.5-56.5T160-800h640q33 0 56.5 23.5T880-720v480q0 33-23.5 56.5T800-160H160Zm320-280L160-640v400h640v-400L480-440Zm0-80 320-200H160l320 200ZM160-640v-80 480-400Z"/></svg>
                        </a>
                    </li>
                </ul>
            </div>
        </div>
     );
}

export default Header;